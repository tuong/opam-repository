...
let transpose ?m ?n ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.S.Mat.transpose" in
  let m = get_dim1_mat loc a_str a ar m_str m in
  let n = get_dim2_mat loc a_str a ac n_str n in
  let b = create n m in
...
external direct_mat_axpy :
  ...
  yc : int ->
  y : mat ->
  unit = "lacaml_Smat_axpy_stub_bc" "lacaml_Smat_axpy_stub"
...
external direct_gemm_trace :
  ...
  bc : int ->
  b : mat ->
  num_type = "lacaml_Sgemm_trace_stub_bc" "lacaml_Sgemm_trace_stub"
...
external direct_syrk_trace :
  ...
  ac : int ->
  a : mat ->
  num_type = "lacaml_Ssyrk_trace_stub"
...
external direct_symm2_trace :
  ...
  bc : int ->
  b : mat ->
  num_type = "lacaml_Ssymm2_trace_stub_bc" "lacaml_Ssymm2_trace_stub"
