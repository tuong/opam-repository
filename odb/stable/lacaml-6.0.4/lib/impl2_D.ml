...
external direct_dot :
  ...
  incx : int ->
  x : vec
  -> float = "lacaml_Ddot_stub_bc" "lacaml_Ddot_stub"

let dot ?n ?ofsx ?incx ~x ?ofsy ?incy y =
  let loc = "Lacaml.D.dot" in
...
external direct_asum :
  ...
  incx : int ->
  x : vec
  -> float = "lacaml_Dasum_stub"

let asum ?n ?ofsx ?incx x =
  let loc = "Lacaml.D.asum" in
  let ofsx, incx = get_vec_geom loc x_str ofsx incx in
  let n = get_dim_vec loc x_str ofsx incx x n_str n in
...
external direct_ger :
  ...
  ac : int ->
  a : mat
  -> unit = "lacaml_Dger_stub_bc" "lacaml_Dger_stub"

let ger ?m ?n ?(alpha = 1.0) ?ofsx ?incx x ?ofsy ?incy y ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.D.ger" in
...
external direct_syr :
  ...
  ac : int ->
  a : mat
  -> unit = "lacaml_Dsyr_stub_bc" "lacaml_Dsyr_stub"

let syr ?n ?(alpha = 1.0) ?(up = true) ?ofsx ?incx x ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.D.syr" in
...
external direct_lansy :
  ...
  a : mat ->
  work : vec
  -> float = "lacaml_Dlansy_stub_bc" "lacaml_Dlansy_stub"
...
external direct_lamch : char -> float = "lacaml_Dlamch_stub"
...
external direct_orgqr :
  ...
  ac : int ->
  a : mat ->
  int = "lacaml_Dorgqr_stub_bc" "lacaml_Dorgqr_stub"
...
let orgqr_opt_lwork ?m ?n ?k ~tau ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.D.orgqr_opt_lwork" in
...
external direct_ormqr :
  ...
  cc : int ->
  c : mat ->
  int = "lacaml_Dormqr_stub_bc" "lacaml_Dormqr_stub"
...
let ormqr_opt_lwork
  ...
  let loc = "Lacaml.D.ormqr_opt_lwork" in
...
external direct_gecon :
  ...
  norm : char ->
  anorm : float ->
  int * float = "lacaml_Dgecon_stub_bc" "lacaml_Dgecon_stub"
...
external direct_sycon :
  ...
  iwork : int_vec ->
  anorm : float ->
  int * float = "lacaml_Dsycon_stub_bc" "lacaml_Dsycon_stub"
...
external direct_pocon :
  ...
  iwork : int_vec ->
  anorm : float ->
  int * float = "lacaml_Dpocon_stub_bc" "lacaml_Dpocon_stub"
...
external direct_gelsy :
  ...
  bc : int ->
  b : mat
  -> int * int = "lacaml_Dgelsy_stub_bc" "lacaml_Dgelsy_stub"
...
let gelsy_opt_lwork ?m ?n ?(ar = 1) ?(ac = 1) a ?nrhs
    ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.D.gelsy_opt_lwork" in
...
external direct_gelsd :
  ...
  bc : int ->
  b : mat
  -> int * int = "lacaml_Dgelsd_stub_bc" "lacaml_Dgelsd_stub"

let lg2_10 = log10 2.0
let log2 n = log10 n /. lg2_10

let gelsd_smlsiz = ilaenv 9 "DGELSD" " " 0 0 0 0
...
external direct_gelss :
  ...
  bc : int ->
  b : mat
  -> int * int = "lacaml_Dgelss_stub_bc" "lacaml_Dgelss_stub"
...
external direct_gesvd :
  ...
  work : vec ->
  lwork : int
  -> int = "lacaml_Dgesvd_stub_bc" "lacaml_Dgesvd_stub"
...
let gesvd_opt_lwork
  ...
  let loc = "Lacaml.D.gesvd_opt_lwork" in
...
external direct_gesdd :
  ...
  lwork : int ->
  iwork : int_vec
  -> int = "lacaml_Dgesdd_stub_bc" "lacaml_Dgesdd_stub"
...
let gesdd_get_opt_lwork loc jobz ?iwork m n ar ac a s ur uc u vtr vtc vt =
  ...
  if info = 0 then
    (* FIXME: LAPACK bug??? *)
    (* int_of_float64 work.{1} *)
    max (int_of_float64 work.{1}) (gesdd_min_lwork_char jobz ~m ~n)
...
let gesdd_opt_lwork
    ...
    ?iwork ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.D.gesdd_opt_lwork" in
...
external direct_geev :
  ...
  work : vec ->
  lwork : int
  -> int = "lacaml_Dgeev_stub_bc" "lacaml_Dgeev_stub"
...
let geev_opt_lwork
    ...
    ?(ofswi = 1) ?wi
    ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.D.geev_opt_lwork" in
...
external direct_syev :
  ...
  work : vec ->
  lwork : int
  -> int = "lacaml_Dsyev_stub_bc" "lacaml_Dsyev_stub"
...
let syev_opt_lwork ?n ?(vectors = false) ?(up = true) ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.D.syev_opt_lwork" in
...
external direct_syevd :
  ...
  iwork : int_vec ->
  liwork : int
  -> int = "lacaml_Dsyevd_stub_bc" "lacaml_Dsyevd_stub"
...
let syevd_opt_l_li_work ?n ?(vectors = false) ?(up = true)
    ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.D.syevd_opt_l_li_work" in
  let n, jobz, uplo = xxev_get_params loc ar ac a n vectors up in
...
external direct_sbev :
  ...
  work : vec
  -> int = "lacaml_Dsbev_stub_bc" "lacaml_Dsbev_stub"
...
external direct_syevr :
  ...
  iwork : int_vec ->
  liwork : int
  -> int * int = "lacaml_Dsyevr_stub_bc" "lacaml_Dsyevr_stub"
...
let syevr_opt_l_li_work
    ...
    ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.D.syevr_opt_l_li_work" in
  let n, jobz, uplo = xxev_get_params loc ar ac a n vectors up in
...
external direct_sygv :
  ...
  work : vec ->
  lwork : int
  -> int = "lacaml_Dsygv_stub_bc" "lacaml_Dsygv_stub"
...
let sygv_opt_lwork
  ...
  let loc = "Lacaml.D.sygv_opt_lwork" in
  let n, jobz, uplo = xxev_get_params loc ar ac a n vectors up in
...
external direct_sbgv :
  ...
  z : mat ->
  work : vec
  -> int = "lacaml_Dsbgv_stub_bc" "lacaml_Dsbgv_stub"
