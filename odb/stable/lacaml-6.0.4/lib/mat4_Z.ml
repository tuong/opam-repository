...
external direct_transpose_copy :
  ...
  bc : int ->
  b : mat ->
  unit = "lacaml_Ztranspose_copy_stub_bc" "lacaml_Ztranspose_copy_stub"

let transpose_copy ?m ?n ?(ar = 1) ?(ac = 1) a ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.Z.Mat.transpose_copy" in
...
let transpose ?m ?n ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.Z.Mat.transpose" in
  let m = get_dim1_mat loc a_str a ar m_str m in
  let n = get_dim2_mat loc a_str a ac n_str n in
  let b = create n m in
...
let packed ?(up = true) ?n ?(ar = 1) ?(ac = 1) (a : mat) =
  let loc = "Lacaml.Z.Mat.packed" in
  let n = get_n_of_square loc a_str ar ac a n in
...
let unpacked ?(up = true) ?n (src : vec) =
  let loc = "Lacaml.Z.Mat.unpacked" in
...
external direct_scal_mat :
  ...
  ac : int ->
  a : mat ->
  unit = "lacaml_Zscal_mat_stub_bc" "lacaml_Zscal_mat_stub"

let scal ?m ?n alpha ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.Z.Mat.scal" in
  let m = get_dim1_mat loc a_str a ar m_str m in
  let n = get_dim2_mat loc a_str a ac n_str n in
...
external direct_scal_cols :
  ...
  ofs : int ->
  alphas : vec ->
  unit = "lacaml_Zscal_cols_stub_bc" "lacaml_Zscal_cols_stub"

let scal_cols ?m ?n ?(ar = 1) ?(ac = 1) a ?ofs alphas =
  let loc = "Lacaml.Z.Mat.scal_cols" in
  let m = get_dim1_mat loc a_str a ar m_str m in
  let n = get_dim2_mat loc a_str a ac n_str n in
...
external direct_scal_rows :
  ...
  ac : int ->
  a : mat ->
  unit = "lacaml_Zscal_rows_stub_bc" "lacaml_Zscal_rows_stub"

let scal_rows ?m ?n ?ofs alphas ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.Z.Mat.scal_rows" in
  let m = get_dim1_mat loc a_str a ar m_str m in
  let n = get_dim2_mat loc a_str a ac n_str n in
  let ofs = get_ofs loc alphas_str ofs in
...
external direct_mat_axpy :
  ...
  yc : int ->
  y : mat ->
  unit = "lacaml_Zmat_axpy_stub_bc" "lacaml_Zmat_axpy_stub"

let axpy ?m ?n ?(alpha = one) ?(xr = 1) ?(xc = 1) ~x ?(yr = 1) ?(yc = 1) y =
  let loc = "Lacaml.Z.Mat.axpy" in
...
external direct_gemm_diag :
  ...
  alpha : num_type ->
  beta : num_type ->
  unit = "lacaml_Zgemm_diag_stub_bc" "lacaml_Zgemm_diag_stub"

let gemm_diag ?n ?k ?(beta = zero) ?(ofsy = 1) ?y
  ?(transa = `N) ?(alpha = one) ?(ar = 1) ?(ac = 1) a
  ?(transb = `N) ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.Z.Mat.gemm_diag" in
...
external direct_syrk_diag :
  ...
  alpha : num_type ->
  beta : num_type ->
  unit = "lacaml_Zsyrk_diag_stub_bc" "lacaml_Zsyrk_diag_stub"

let syrk_diag ?n ?k ?(beta = zero) ?(ofsy = 1) ?y
  ?(trans = `N) ?(alpha = one) ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.Z.Mat.syrk_diag" in
...
external direct_gemm_trace :
  ...
  bc : int ->
  b : mat ->
  num_type = "lacaml_Zgemm_trace_stub_bc" "lacaml_Zgemm_trace_stub"

let gemm_trace ?n ?k ?(transa = `N) ?(ar = 1) ?(ac = 1) a
  ?(transb = `N) ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.Z.Mat.gemm_trace" in
  let n = get_rows_mat_tr loc a_str a ar ac transa n_str n in
  let n = get_cols_mat_tr loc b_str b br bc transb n_str (Some n) in
  let k = get_inner_dim loc a_str a ar ac transa b_str b br bc transb k_str k in
  let transa = get_trans_char transa in
...
external direct_syrk_trace :
  ...
  ac : int ->
  a : mat ->
  num_type = "lacaml_Zsyrk_trace_stub"

let syrk_trace ?n ?k ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.Z.Mat.syrk_trace" in
...
external direct_symm2_trace :
  ...
  bc : int ->
  b : mat ->
  num_type = "lacaml_Zsymm2_trace_stub_bc" "lacaml_Zsymm2_trace_stub"

let symm2_trace
  ?n
  ?(upa = true) ?(ar = 1) ?(ac = 1) a
  ?(upb = true) ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.Z.Mat.symm2_trace" in
  let n = get_n_of_square loc a_str ar ac a n in
...
let fold_cols coll ?n ?(ac = 1) acc a =
  let loc = "Lacaml.Z.Mat.fold_cols" in
  let n = get_dim2_mat loc a_str a ac n_str n in
  let rec loop i acc =
    if i = n then acc
