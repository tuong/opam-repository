...
external direct_dotu :
  ...
  incx : int ->
  x : vec ->
  num_type = "lacaml_Zdotu_stub_bc" "lacaml_Zdotu_stub"
...
external direct_dotc :
  ...
  incx : int ->
  x : vec ->
  num_type = "lacaml_Zdotc_stub_bc" "lacaml_Zdotc_stub"
...
let dotu = gen_dot "Lacaml.Z.dotu" direct_dotu
let dotc = gen_dot "Lacaml.Z.dotc" direct_dotc
...
external direct_gecon :
  ...
  norm : char ->
  anorm : float ->
  int * float = "lacaml_Zgecon_stub_bc" "lacaml_Zgecon_stub"
...
external direct_sycon :
  ...
  work : vec ->
  anorm : float ->
  int * float = "lacaml_Zsycon_stub_bc" "lacaml_Zsycon_stub"

let sycon_min_lwork n = 2 * n

let sycon ?n ?(up = true) ?ipiv ?anorm ?work ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.Z.sycon" in
  let n = get_n_of_a loc ar ac a n in
  let uplo = get_uplo_char up in
  let work, _lwork =
    get_work
...
external direct_pocon :
  ...
  rwork : rvec ->
  anorm : float ->
  int * float = "lacaml_Zpocon_stub_bc" "lacaml_Zpocon_stub"
...
external direct_gesvd :
  ...
  lwork : int ->
  rwork : rvec ->
  int = "lacaml_Zgesvd_stub_bc" "lacaml_Zgesvd_stub"
...
let gesvd_get_opt_lwork loc jobu jobvt m n ar ac a s ur uc u vtr vtc vt =
  ...
  in
  if info = 0 then int_of_float64 work.{1}.re
...
let gesvd_opt_lwork
  ...
  let loc = "Lacaml.Z.gesvd_opt_lwork" in
...
external direct_geev :
  ...
  lwork : int ->
  rwork : vec ->
  int = "lacaml_Zgeev_stub_bc" "lacaml_Zgeev_stub"
...
let geev_opt_lwork
    ...
    ?(ofsw = 1) ?w
    ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.Z.geev_opt_lwork" in
