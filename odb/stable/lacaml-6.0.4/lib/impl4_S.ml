...
external direct_swap :
  ...
  incx : int ->
  x : vec ->
  unit = "lacaml_Sswap_stub_bc" "lacaml_Sswap_stub"

let swap ?n ?ofsx ?incx ~x ?ofsy ?incy y =
  let loc = "Lacaml.S.swap" in
...
external direct_scal :
  ...
  incx : int ->
  x : vec ->
  unit = "lacaml_Sscal_stub"

let scal ?n alpha ?ofsx ?incx x =
  let loc = "Lacaml.S.scal" in
  let ofsx, incx = get_vec_geom loc x_str ofsx incx in
  let n = get_dim_vec loc x_str ofsx incx x n_str n in
...
external direct_nrm2 :
  ...
  incx : int ->
  x : vec
  -> float = "lacaml_Snrm2_stub"

let nrm2 ?n ?ofsx ?incx x =
  let loc = "Lacaml.S.nrm2" in
  let ofsx, incx = get_vec_geom loc x_str ofsx incx in
  let n = get_dim_vec loc x_str ofsx incx x n_str n in
...
external direct_axpy :
  ...
  incx : int ->
  x : vec ->
  unit = "lacaml_Saxpy_stub_bc" "lacaml_Saxpy_stub"

let axpy ?n ?(alpha = one) ?ofsx ?incx ~x ?ofsy ?incy y =
  let loc = "Lacaml.S.axpy" in
  let ofsx, incx = get_vec_geom loc x_str ofsx incx in
  let ofsy, incy = get_vec_geom loc y_str ofsy incy in
  let n = get_dim_vec loc x_str ofsx incx x n_str n in
  check_vec loc y_str y (ofsy + (n - 1) * abs incy);
...
external direct_iamax :
  ...
  incx : int ->
  x : vec ->
  int = "lacaml_Siamax_stub"

let iamax ?n ?ofsx ?incx x =
  let loc = "Lacaml.S.iamax" in
  let ofsx, incx = get_vec_geom loc x_str ofsx incx in
  let n = get_dim_vec loc x_str ofsx incx x n_str n in
  direct_iamax ~n ~ofsx ~incx ~x

let amax ?n ?ofsx ?incx x =
  let loc = "Lacaml.S.amax" in
  let ofsx, incx = get_vec_geom loc x_str ofsx incx in
  let n = get_dim_vec loc x_str ofsx incx x n_str n in
...
external direct_symv :
  ...
  incx : int ->
  x : vec ->
  unit = "lacaml_Ssymv_stub_bc" "lacaml_Ssymv_stub"

let symv ?n ?(beta = zero) ?ofsy ?incy ?y ?(up = true) ?(alpha = one)
      ?(ar = 1) ?(ac = 1) a ?ofsx ?incx x =
  let loc = "Lacaml.S.symv" in
...
external direct_trmv :
  ...
  incx : int ->
  x : vec ->
  unit = "lacaml_Strmv_stub_bc" "lacaml_Strmv_stub"

let trmv
      ?n ?(trans = `N) ?(diag = `N) ?(up = true)
      ?(ar = 1) ?(ac = 1) a ?ofsx ?incx x =
  let loc = "Lacaml.S.trmv" in
...
external direct_trsv :
  ...
  incx : int ->
  x : vec ->
  unit = "lacaml_Strsv_stub_bc" "lacaml_Strsv_stub"

let trsv
      ?n ?(trans = `N) ?(diag = `N) ?(up = true)
      ?(ar = 1) ?(ac = 1) a ?ofsx ?incx x =
  let loc = "Lacaml.S.trsv" in
  let n, ofsx, incx, uplo, trans, diag =
    trXv_get_params loc ar ac a n ofsx incx x up trans diag
  in
...
external direct_tpmv :
  ...
  incx : int ->
  x : vec ->
  unit = "lacaml_Stpmv_stub_bc" "lacaml_Stpmv_stub"

let tpmv ?n ?(trans = `N) ?(diag = `N) ?(up = true) ?ofsap ap ?ofsx ?incx x =
  let loc = "Lacaml.S.tpmv" in
...
external direct_tpsv :
  ...
  incx : int ->
  x : vec ->
  unit = "lacaml_Stpsv_stub_bc" "lacaml_Stpsv_stub"

let tpsv ?n ?(trans = `N) ?(diag = `N) ?(up = true) ?ofsap ap ?ofsx ?incx x =
  let loc = "Lacaml.S.tpsv" in
  let n, ofsap, ofsx, incx, uplo, trans, diag =
    tpXv_get_params loc ofsap ap ?n ofsx incx x up trans diag
  in
...
external direct_trmm :
  ...
  b : mat ->
  alpha : num_type ->
  unit = "lacaml_Strmm_stub_bc" "lacaml_Strmm_stub"

let trmm ?m ?n ?(side = `L) ?(up = true) ?(transa = `N) ?(diag = `N)
      ?(alpha = one) ?(ar = 1) ?(ac = 1) ~a ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.trmm" in
...
external direct_trsm :
  ...
  b : mat ->
  alpha : num_type ->
  unit = "lacaml_Strsm_stub_bc" "lacaml_Strsm_stub"

let trsm ?m ?n ?(side = `L) ?(up = true) ?(transa = `N) ?(diag = `N)
      ?(alpha = one) ?(ar = 1) ?(ac = 1) ~a ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.trsm" in
  let m, n, side, uplo, transa, diag =
    trXm_get_params loc ar ac a br bc b m n side up transa diag
  in
...
external direct_lacpy :
  ...
  bc : int ->
  b : mat ->
  unit = "lacaml_Slacpy_stub_bc" "lacaml_Slacpy_stub"

let lacpy ?uplo ?m ?n ?(br = 1) ?(bc = 1) ?b ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.S.lacpy" in
  let m = get_dim1_mat loc a_str a ar m_str m in
  let n = get_dim2_mat loc a_str a ac n_str n in
  let b, br, bc =
    match b with
...
external direct_lassq :
  ...
  scale : float ->
  sumsq : float ->
  float * float = "lacaml_Slassq_stub_bc" "lacaml_Slassq_stub"

let lassq ?n ?(scale = 0.) ?(sumsq = 1.) ?ofsx ?incx x =
  let loc = "Lacaml.S.lassq" in
  let ofsx, incx = get_vec_geom loc x_str ofsx incx in
  let n = get_dim_vec loc x_str ofsx incx x n_str n in
...
external direct_larnv :
  ...
  ofsx : int ->
  x : vec ->
  unit = "lacaml_Slarnv_stub_bc" "lacaml_Slarnv_stub"

let larnv ?idist ?ofsiseed ?iseed ?n ?ofsx ?x () =
  let loc = "Lacaml.S.larnv" in
  let idist =
    match idist with
    | Some `Uniform0 -> 1
    | Some `Uniform1 -> 2
...
external direct_lauum :
  ...
  ac : int ->
  a : mat ->
  unit = "lacaml_Slauum_stub"

let lauum ?(up = true) ?n ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.S.lauum" in
...
external direct_getrf :
  ...
  a : mat ->
  ipiv : int_vec ->
  int = "lacaml_Sgetrf_stub_bc" "lacaml_Sgetrf_stub"

let getrf ?m ?n ?ipiv ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.S.getrf" in
...
external direct_getrs :
  ...
  b : mat ->
  ipiv : int_vec ->
  int = "lacaml_Sgetrs_stub_bc" "lacaml_Sgetrs_stub"

let getrs
    ?n ?ipiv ?(trans = `N) ?(ar = 1) ?(ac = 1) a ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.getrs" in
...
external direct_getri :
  ...
  work : vec ->
  lwork : int ->
  int = "lacaml_Sgetri_stub_bc" "lacaml_Sgetri_stub"
...
external direct_sytrf :
  ...
  work : vec ->
  lwork : int ->
  int = "lacaml_Ssytrf_stub_bc" "lacaml_Ssytrf_stub"
...
let sytrf_opt_lwork ?n ?(up = true) ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.S.sytrf_opt_lwork" in
  let uplo = get_uplo_char up in
  let n = get_n_of_a loc ar ac a n in
...
external direct_sytrs :
  ...
  b : mat ->
  ipiv : int_vec ->
  int = "lacaml_Ssytrs_stub_bc" "lacaml_Ssytrs_stub"

let sytrs
      ?n ?(up = true) ?ipiv ?(ar = 1) ?(ac = 1) a ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.sytrs" in
  let uplo = get_uplo_char up in
  let n, nrhs = xxtrs_get_params loc ar ac a n br bc b nrhs in
  let ipiv =
...
external direct_sytri :
  ...
  ipiv : int_vec ->
  work : vec ->
  int = "lacaml_Ssytri_stub_bc" "lacaml_Ssytri_stub"

let sytri_min_lwork n = n

let sytri ?n ?(up = true) ?ipiv ?work ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.S.sytri" in
  let uplo = get_uplo_char up in
  let n = get_n_of_a loc ar ac a n in
  let work, _lwork =
    get_work
...
external direct_potrs :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Spotrs_stub_bc" "lacaml_Spotrs_stub"

let potrs
      ?n ?(up = true) ?(ar = 1) ?(ac = 1) a ?nrhs ?(br = 1) ?(bc = 1)
      ?(factorize = true) ?jitter b =
  let loc = "Lacaml.S.potrs" in
  let uplo = get_uplo_char up in
  let n, nrhs = xxtrs_get_params loc ar ac a n br bc b nrhs in
...
external direct_trtrs :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Strtrs_stub_bc" "lacaml_Strtrs_stub"

let trtrs
      ?n ?(up = true) ?(trans = `N) ?(diag = `N)
      ?(ar = 1) ?(ac = 1) a ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.trtrs" in
  let uplo = get_uplo_char up in
  let trans = get_trans_char trans in
...
external direct_trtri :
  ...
  ac : int ->
  a : mat ->
  int = "lacaml_Strtri_stub_bc" "lacaml_Strtri_stub"

let trtri ?n ?(up = true) ?(diag = `N) ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.S.trtri" in
  let n = get_n_of_a loc ar ac a n in
  let uplo = get_uplo_char up in
  let diag = get_diag_char diag in
...
external direct_tbtrs :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Stbtrs_stub_bc" "lacaml_Stbtrs_stub"

let tbtrs
    ?n ?kd ?(up = true) ?(trans = `N) ?(diag = `N)
    ?(abr = 1) ?(abc = 1) ab ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.tbtrs" in
  let uplo = get_uplo_char up in
  let trans = get_trans_char trans in
  let diag = get_diag_char diag in
...
external direct_geqrf :
  ...
  work : vec ->
  lwork : int ->
  unit = "lacaml_Sgeqrf_stub_bc" "lacaml_Sgeqrf_stub"
...
let geqrf_opt_lwork ?m ?n ?(ar = 1) ?(ac = 1) a =
  let loc = "Lacaml.S.geqrf_opt_lwork" in
  let m, n = geXrf_get_params loc m n ar ac a in
...
external direct_gesv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Sgesv_stub_bc" "lacaml_Sgesv_stub"

let gesv ?n ?ipiv ?(ar = 1) ?(ac = 1) a ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.gesv" in
...
external direct_gbsv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Sgbsv_stub_bc" "lacaml_Sgbsv_stub"

let gbsv ?n ?ipiv ?(abr = 1) ?(abc = 1) ab kl ku ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.gbsv" in
  let n = get_dim2_mat loc ab_str ab abc n_str n in
...
external direct_gtsv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Sgtsv_stub_bc" "lacaml_Sgtsv_stub"

let gtsv ?n ?ofsdl dl ?ofsd d ?ofsdu du ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.gtsv" in
...
external direct_posv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Sposv_stub_bc" "lacaml_Sposv_stub"

let posv ?n ?(up = true) ?(ar = 1) ?(ac = 1) a ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.posv" in
  let n, nrhs = xxsv_get_params loc ar ac a n br bc b nrhs in
  let info =
...
external direct_ppsv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Sppsv_stub_bc" "lacaml_Sppsv_stub"

let ppsv ?n ?(up = true) ?ofsap ap ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.ppsv" in
...
external direct_pbsv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Spbsv_stub_bc" "lacaml_Spbsv_stub"

let pbsv ?n ?(up = true) ?kd ?(abr = 1) ?(abc = 1) ab
      ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.pbsv" in
...
external direct_ptsv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Sptsv_stub_bc" "lacaml_Sptsv_stub"

let ptsv ?n ?ofsd d ?ofse e ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.ptsv" in
  let ofsd = get_ofs loc d_str ofsd in
...
external direct_sysv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Ssysv_stub_bc" "lacaml_Ssysv_stub"
...
let sysv_opt_lwork ?n ?(up = true) ?(ar = 1) ?(ac = 1) a
      ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.sysv_opt_lwork" in
  let n, nrhs = xxsv_get_params loc ar ac a n br bc b nrhs in
  let uplo = get_uplo_char up in
...
external direct_spsv :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Sspsv_stub_bc" "lacaml_Sspsv_stub"

let spsv ?n ?(up = true) ?ipiv ?ofsap ap ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.spsv" in
  let ofsap = get_ofs loc ap_str ofsap in
  let n = get_dim_mat_packed loc ap_str ofsap ap n_str n in
  let nrhs = get_nrhs_of_b loc n br bc b nrhs in
  let ipiv = xxsv_get_ipiv loc ipiv n in
...
external direct_gels :
  ...
  bc : int ->
  b : mat ->
  int = "lacaml_Sgels_stub_bc" "lacaml_Sgels_stub"
...
let gels_opt_lwork ?m ?n ?(trans = `N) ?(ar = 1) ?(ac = 1) a
      ?nrhs ?(br = 1) ?(bc = 1) b =
  let loc = "Lacaml.S.gels_opt_lwork" in
