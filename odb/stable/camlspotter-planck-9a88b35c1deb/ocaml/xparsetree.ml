...
type constant =
  ...
  | Const_nativeint of nativeint with sexp

type rec_flag = Nonrecursive | Recursive | Default with sexp

type direction_flag = Upto | Downto with sexp

type private_flag = Private | Public with sexp

type mutable_flag = Immutable | Mutable with sexp

type virtual_flag = Virtual | Concrete with sexp
end
open Asttypes
