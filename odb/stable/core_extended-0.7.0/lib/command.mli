...
(** Exactly like [run] except that it takes [unit] () as a final argument to
    delay the call until [unit] is passed. For example:

    [let main = Command.run_staged command in
    print_endline "About to call run!";
    main ()]
  *)
val run_staged :
  ?argv:string list
